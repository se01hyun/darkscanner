...
const COUNTDOWN_TEXT_PATTERN =
  /(오늘\s*(만|한정|까지|중)|마감\s*(임박|까지|[0-9])|한\s*정\s*판매|한\s*정\s*수량|지금\s*만|이\s*가격\s*마지막|[0-9]+\s*(시간|분|초)\s*(후|뒤|남음|남았|이내|안에|限|한정)|D\s*[-–]\s*[0-9]+|[0-9]+일\s*(남음|후\s*종료|후\s*마감|째)|오늘\s*[0-9]+시\s*(까지|마감|종료)|자정\s*(까지|마감)|세일\s*(마감|종료)\s*(임박|[0-9]+)|타임\s*(세일|딜|특가)|flash\s*sale|ends?\s*in\s*[0-9])/i;
