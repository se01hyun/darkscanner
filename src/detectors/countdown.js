...
/** 시간제한 관련 텍스트 패턴 */
// 주의: "남음이 있으며", "남았습니다만" 등 조사·어미가 붙으면 서술어(descriptive)이지 카운트다운이 아님.
// negative lookahead (?![이은가를도만의에서]) 로 조사 결합 케이스를 제외한다.
const COUNTDOWN_TEXT_PATTERN =
  /(오늘\s*(만|한정|까지|중)|마감\s*(임박|까지|[0-9])|한\s*정\s*판매|한\s*정\s*수량|지금\s*만|이\s*가격\s*마지막|[0-9]+\s*(시간|분|초)\s*(후|뒤|이내|안에|限|한정)|[0-9]+\s*(시간|분|초)\s*남음(?![이은가를도만의에서])|[0-9]+\s*(시간|분|초)\s*남았(?![습으])|D\s*[-–]\s*[0-9]+|[0-9]+일\s*(남음(?![이은가를도만의에서])|후\s*종료|후\s*마감|째)|오늘\s*(오전|오후)?\s*[0-9]+시\s*(까지|마감|종료)|자정\s*(까지|마감)|세일\s*(마감|종료)\s*(임박|[0-9]+)|타임\s*(세일|딜|특가)|flash\s*sale|ends?\s*in\s*[0-9])/i;
...
function isCountdownText(el) {
  ...
  // 쇼핑몰 카운트다운은 전용 짧은 요소에 표시됨.
  // 긴 단락(>150자)은 패턴어가 일반 문맥에 묻힌 경우로 간주하여 제외.
  if (text.length > 150) return false;
  return COUNTDOWN_TEXT_PATTERN.test(text);
}
