...
const SOCIAL_PROOF_PATTERN = /\d+[\s,]*(명|개|건)[\s가-힣]*(보[는고]|구매|주문|담[았]?|관심|확인|신청|조회|리뷰|후기|평가|선택|클릭|방문|입력|결제|완료)|(최근\s*\d+\s*(명|개|시간|일)[\s가-힣]*)|(지금\s*(막|이\s*순간|구매|담고|주문))|(한정\s*\d+\s*(개|명|석|자리))|(품절\s*임박|재고\s*\d+\s*개?\s*(남음|뿐|밖에))|(오늘\s*\d+\s*(명|개|건)\s*(구매|주문|조회))/i;
